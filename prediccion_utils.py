...
def predecir_registro(precio_t1, precio_t2, mes, producto, provincia, le_prod, le_prov, le_target, modelo, features, categoria_perecedero=0):
    ...
    utilizando distancias porcentuales a los promedios móviles.
    """
    variacion_t2_t1 = round(((precio_t1 - precio_t2) / precio_t2) * 100, 2) if (precio_t2 and precio_t2 > 0) else 0.0
    
    # Cálculo de distancias porcentuales respecto al promedio móvil
    pm2 = (precio_t1 + precio_t2) / 2.0 if (precio_t1 and precio_t2) else 0.0
    distancia_pm2_pct = round(((precio_t1 - pm2) / pm2) * 100, 2) if (pm2 > 0 and precio_t1 > 0) else 0.0
    distancia_pm3_pct = distancia_pm2_pct  # Aproximación al no tener t3 en entrada manual
    ...
    row_dict = {
        ...
        "distancia_pm2_pct": distancia_pm2_pct,
        "distancia_pm3_pct": distancia_pm3_pct,
        "mes": mes,
...
def predecir_dataframe(df, modelo, le_target, features):
    ...
    """
    df_eval = df.dropna(subset=["precio_t2", "variacion_t2_t1", "distancia_pm2_pct", "distancia_pm3_pct"]).copy()
